...
| ID | Story | Acceptance criteria | Tier |
|----|-------|---------------------|------|
| **FORGE-010** | **Guided onboarding** — URL capture, canonical domain, “connect PostHog / Segment”, validate integration, ingest sample window estimate. | Time-to-first-`trustworthy:true` or explicit “blocked” receipts < 24h assisted. | P0 |
| **FORGE-011** | **Single-site dashboard shell** — one site overview: health / last sync / gate status / CTA **Run audit** + **Export receipt**. | Matches vision: improver cockpit, not page builder canvas. | P1 |
...
Shipped primitives: `/phase2` UI snapshot, **`POST /api/phase2/insights/receipt`** (see codebase) for credibility exports.
...
| ID | Story | Acceptance criteria | Tier |
|----|-------|---------------------|------|
| **FORGE-060** | **Hypothesis linkage** — user pins PostHog insight URL or funnel id to a finding (`externalReceiptRef`). | Exports cite customer-owned dashboard for lift claims. | P1 |
| **FORGE-061** | **Bet status** — `planned` / `shipped` / `measured` with owner + date; no full PM suite. | CSV export for QBR. | P2 |
...
| **3–4** | FORGE-020, 010, 041 — ingestion reliability + onboarding + quotas |
| **5–6** | FORGE-040 Stripe + plan enforcement + staging (071) |
| **7–8** | FORGE-032 persisted runs + 012 methodology + observability sparkline (072 starter) |
