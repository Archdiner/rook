...
function parseBearerForgeKey(request: Request): string | null {
  ...
  if (!token.startsWith('forge_sk_')) return null;
  return token;
}
...
export async function resolveForgeActor(
  request: Request,
  options?: {
    bodyOrganizationId?: unknown;
  ...
  { ok: true; actor: ForgeActor } | { ok: false; response: NextResponse }
> {
  ...
  if (apiToken) {
    ...
    if (!validated) {
      return {
        ok: false,
        response: unauthorized('Invalid or revoked Forge API key.', 'INVALID_API_KEY'),
      };
    }
