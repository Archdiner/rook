...
import { forgeSiteMeta } from '@/lib/db/schema';
import { createPhase1Repository } from '@/lib/phase1';

async function getOrInit(siteId: string, organizationId: string) {
  const db = getDb();
  const rows = await db.select().from(forgeSiteMeta).where(eq(forgeSiteMeta.siteId, siteId)).limit(1);
...
export async function PATCH(request: Request) {
  try {
    ...
    const set: Partial<typeof forgeSiteMeta.$inferInsert> = { updatedAt: new Date() };
    ...
    await db
      ...
      .onConflictDoUpdate({ target: forgeSiteMeta.siteId, set });
