...
export async function GET(request: Request) {
  try {
    ...
    const db = getDb();
    const conditions = [eq(forgeFindings.siteId, siteId)];
    if (statusFilter) conditions.push(eq(forgeFindings.status, statusFilter));

    const rows = await db
      .select()
      .from(forgeFindings)
      .where(and(...conditions))
      .orderBy(desc(forgeFindings.priorityScore))
      .limit(100);
