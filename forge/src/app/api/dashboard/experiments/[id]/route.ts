...
export async function GET(
  ...
  try {
    ...
    const rows = await db
      ...
      .where(
        and(
          eq(forgeExperiments.id, id),
          eq(forgeExperiments.organizationId, actorResult.actor.organizationId)
        )
      )
      .limit(1);
    ...
    if (exp.findingId) {
      const fr = await db
        .select()
        .from(forgeFindings)
        .where(eq(forgeFindings.id, exp.findingId))
        .limit(1);
      finding = fr[0] ?? null;
    }
...
export async function PATCH(
  ...
  try {
    ...
    await db.update(forgeExperiments).set(update as any).where(eq(forgeExperiments.id, id));

    const updated = await db
      .select()
      .from(forgeExperiments)
      .where(eq(forgeExperiments.id, id))
      .limit(1);
    ...
    if (
      ...
      await db
        ...
        .where(
          and(
            eq(forgeFindings.id, updated[0].findingId),
