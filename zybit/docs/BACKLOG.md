...
| **Zybit-082** | Chi-squared significance — chi-squared test for proportion comparison (not pooled-variance z-test); Welch's t-test for continuous metrics; output: p-value, confidence. | Pure function with unit tests covering edge cases (zero conversions, very small samples). **Math shipped — `5951a99` (`stats.ts`), alpha param fix in `b09a212`. Unit tests shipped in `__tests__/stats.test.ts` (77 passing). Monte Carlo pipeline simulation in `__tests__/stats.simulation.test.ts` measured empirical false-positive rate at ~14% under the null (vs nominal 5%) — repeated-peeking inflation; closing it requires α-spending or reduced peek cadence (separate, larger workstream).** | P0 |
...
| Block | Focus | Stories |
|-------|-------|---------|
| **Week-1 tail (small)** | Close shipped-but-incomplete acceptance criteria | `X-Frame-Options` strip + side-by-side iframe UI (Zybit-112), "last computed at" surface (Zybit-086), Resend notification on auto-stop (Zybit-084). `stats.ts` unit tests (Zybit-082) landed; pipeline FP-rate inflation from repeated peeking surfaced by the Monte Carlo is a separate, larger workstream (α-spending or reduced peek cadence). |
