...
**Rendering notes:**
- Funnel: render as two horizontal bars, `Submitted` bar is shorter and flagged red/amber
- Drop-off percentage should be computed and shown: 892 → 178 = 80% drop
- Items list shows field names with a required/optional badge — flagged fields get amber highlight
- `proposedFix` renders below the diagram as a callout sentence
...
**Rendering notes:**
- "Most-clicked CTA" and "Visually heaviest CTA" are the KEY pair — render them prominently side by side
...
- Background: `bg-[#111]` text `white` for ecommerce/revenue goals
- Background: `bg-amber-50` border `amber-100` for engagement/growth goals
- The `basis` string is the napkin math — show it, small, below the formatted value
