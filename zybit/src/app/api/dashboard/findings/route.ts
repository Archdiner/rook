    ...
    if (auditFindings.length > 0) {
      const values = auditFindings.map((f) => {
        ...
        return {
          ...
          snapshotDiagram: (f.snapshotDiagram ? JSON.parse(JSON.stringify(f.snapshotDiagram)) : null) as Record<string, unknown> | null,
          refs: f.refs ?? null,
      ...
      await db
        ...
        .onConflictDoUpdate({
          ...
          set: {
            ...
            snapshotDiagram: sql`excluded.snapshot_diagram`,
