  ...
  return `
    <div style="font-family: Georgia, serif; max-width: 640px; margin: 0 auto; color: #0E0C09; background: #FAFAF8; padding: 32px;">
      <div style="border-bottom: 2px solid #0E0C09; padding-bottom: 16px; margin-bottom: 24px;">
        ...
        <p style="font-size: 13px; color: #5A4F3A; margin: 6px 0 0; letter-spacing: 0.1em; text-transform: uppercase;">Zybit · Phase 0 · ${timestamp}</p>
...
export async function POST(request: Request) {
  try {
    ...
    const { error } = await getResendClient().emails.send({
      from: 'Zybit Discovery <onboarding@resend.dev>',
      to: RECIPIENT,
      subject: `New Zybit Discovery Response — ${subjectUrl}`,
